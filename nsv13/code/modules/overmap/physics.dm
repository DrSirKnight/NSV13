...
/obj/structure/overmap/proc/fire_projectile(proj_type, atom/target, speed=null, user_override=null, lateral=FALSE, ai_aim = FALSE, miss_chance=5, max_miss_distance=5, broadside=FALSE, bullethell=FALSE, bullethell_angle=0) //Fire one shot. Used for big, hyper accelerated shots rather than PDCs
	if(!z || QDELETED(src))
		return FALSE
	var/turf/T = get_center()
	...
	spawn()
		...
		if(bullethell)
			proj.setAngle(bullethell_angle)
